...
mod_lims = [0.01, 100, 0.1, 1000, 0., 2. * np.pi, 0., 2. * np.pi, 0.0, 1.0, 0.0001,100., -50.0,50.0]
mod_lims[2] = np.log(mod_lims[2])
mod_lims[3] = np.log(mod_lims[3])
...
def lnlike(theta, time, rv, err):
    ...
    invsigma2 = 1. / np.sqrt(err**2 + jitt**2)
    counter += 1
    if counter == 100:
        counter = 0
        pbar.update(100)

    return -0.5 * np.sum(((rv - MODEL) ** 2 * invsigma2) - np.log(invsigma2))
...
def lnprior_flat(theta):
    ...
    if mod_lims[0] <= A <= mod_lims[1] and mod_lims[2] <= P <= mod_lims[3] and mod_lims[4] <= w <= mod_lims[5] and mod_lims[6] <= phase <= mod_lims[7] and mod_lims[8] <= ecc <= mod_lims[9] and  mod_lims[10] <= jitt <= mod_lims[11] and mod_lims[12] <= offset <= mod_lims[13]:
        return 0.0
    per = np.exp(P)  # what's up with this?
...
def lnprior(theta):
    ...
    if mod_lims[0] <= A <= mod_lims[1]:
        ...
        #print(mod_lims[1] * (np.exp(mod_lims[2]) / per) ** (1. / 3) * (1. / np.sqrt(1. - ecc ** 2.)))
        #lp_semi = (semiamp+k0)**(-1.0) / np.log( 1.0+(kmax/k0) * (pmin/period_lp)**(1.0/3.0) * (1.0 / np.sqrt(1.0 - eccentric**2.0)) )
        #lp_amp = (A + mod_lims[0]) ** -1. / np.log(1.0 + mod_lims[1]/mod_lims[0] * (np.exp(mod_lims[2])/np.exp(mod_lims[3]) ** 1./3 * (1./ np.sqrt(1. - ecc ** 2.)) ))
        lp_amp = (A + mod_lims[0]) ** -1. * np.log(1. + mod_lims[1] / mod_lims[0])
    else:
        return - np.inf

    # Period Jeffreys
    if mod_lims[2] <= P <= mod_lims[3]:
        lp_per = 1. / (P * np.log10(np.exp(mod_lims[3]) / np.exp(mod_lims[2]) ))
    else:
        return - np.inf
    ...
    if mod_lims[10] <= jitt <= mod_lims[11]:  # jitmax and not ampmax?
        lp_jit = (jitt + mod_lims[10]) ** (-1.0) / np.log(1. + mod_lims[11] / mod_lims[10])
    else:
        return -np.inf
    # Offset
    if mod_lims[12] <= offset <= mod_lims[13]:
        lp_off = 1.0 / (mod_lims[12] - mod_lims[13])
    else:
        return - np.inf
